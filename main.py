...
#Function to clear text in a text box
def cleartext():
    T.delete("1.0","end")
...
#Function to avoid note values greater than 12
def mod(x):
    if x>12:
     return(x%12)
    else:
...
def closewindow():
    exit()
#Function for producing sound with 3 keys
...
#Function for producing sound with 4 keys
def Play_Sound1(p,q,r,s):
    ...
    play(mixed2)
#Different functions for producing different types of chords
...
def Play_Maj7th(k):
    ...
    key_4=mod(k+11)
    Play_Sound1(key_1,key_2,key_3,key_4)
#Function to identify key and type
def playchord():
 key=str(KEY.get())
 type=str(TYPE.get())
